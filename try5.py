...
def main():
    ...
    episode_avg_waiting_times = []  # 차량 1대당 평균 대기 시간 저장
    ...
    for episode in range(episodes):
        ...
        total_vehicles = 0
        epsilon = max(0.01, 0.08 - 0.01 * (episode / 4))

        # 초기 대기 시간 계산
        prev_waiting_time = get_total_waiting_time()
        ...
        for step in range(num_steps):
            ...
            # 현재 대기 시간 계산 및 보상 계산
            current_waiting_time = get_total_waiting_time()
            r = prev_waiting_time - current_waiting_time  # 대기 시간 감소를 보상
            prev_waiting_time = current_waiting_time

            memory.put((s, a, r, s_prime))

            # 첫 번째 에피소드에서는 학습 진행하지 않음
            if episode > 0:
                train(q, q_target, memory, optimizer)

            s = s_prime
            cumulate_waitingTime += current_waiting_time
